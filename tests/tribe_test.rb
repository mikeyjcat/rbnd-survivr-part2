...
class TestTribe < Minitest::Test
  def setup
    @contestants = %w(carlos walter aparna trinh diego juliana poornima juha
                      sofia julia fernando dena orit colt zhalisa farrin
                      muhammed ari rasha gauri)
  ...
  def test_initialize_puts_tribe_creation
    contestants = %w('person_one', 'person_two')
    contestants.map! { |contestant| Contestant.new(contestant) }
  ...
  def test_tribal_council_returns_who_was_voted_off
    immune = @coyopa.members.first
    assert_equal 1, [@coyopa.tribal_council(immune)].size
  end
end
